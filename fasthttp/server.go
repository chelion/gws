...
func (ctx *RequestCtx) IsConnectClose() bool {
	readBuf := make([]byte,0)
	recvlen,err := ctx.c.Read(readBuf)
	if recvlen < 0 || nil != err{
		return false
	}
	return true
}
...
func (ctx *RequestCtx) NotModified() {
	ctx.Response.Reset()
	ctx.Response.Header.SetServerBytes(ctx.s.getServerName())
...
func (server *Server)WorkerFunc(conn net.Conn)error{
	...
	for {
		...
		if server.ReadTimeout > 0 || server.MaxKeepaliveDuration > 0 {
			lastReadDeadlineTime = server.updateReadDeadline(conn, ctx, lastReadDeadlineTime)
		...
		if !(server.ReduceMemoryUsage || ctx.lastReadDuration > time.Second) || br != nil {
			if br == nil {
		...
		if err == nil {
			...
			err = ctx.Request.readLimitBody(br, maxRequestBodySize, server.GetOnly)
			if br.Buffered() == 0 || err != nil {
				releaseReader(server, br)
				br = nil
			}
		}
		...
		connectionClose = server.DisableKeepalive || ctx.Request.Header.connectionCloseFast()
		...
		if server.MaxRequestsPerConn > 0 && connRequestNum >= uint64(server.MaxRequestsPerConn) {
			ctx.SetConnectionClose()
		}

		if server.WriteTimeout > 0 || server.MaxKeepaliveDuration > 0 {
			lastWriteDeadlineTime = server.updateWriteDeadline(conn, ctx, lastWriteDeadlineTime)
		}
		...
		if hijackHandler != nil {
			...
			go hijackConnHandler(hjr, conn, server, hijackHandler)
			hijackHandler = nil
			err = errHijacked
			break
		}
...
func (workerJob *WokerJob)Run(){
	err := workerJob.server.WorkerFunc(workerJob.conn)
	if nil != err{
		workerJob.conn = nil
	}
}
...
func (s *Server) Serve(ln net.Listener) error {
	...
	workPool,_ := utils.NewWorkerPool(int64(maxWorkersCount))
	...
	for {
		...
		workPool.Work(utils.Job(&WokerJob{server:s,conn:c}))
	}
}
...
func (s *Server)SetServerName(servername string){
	s.serverName.Store([]byte(servername))
}
